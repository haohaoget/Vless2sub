			...
			if(url.searchParams.get('client') && (url.searchParams.get('client').includes('ip'))){
				...
				cftctapi.map(line => {
					...
					if (match){
						...
						// }
						path = encodeURIComponent(path);
						const vlessLink = `vless://${uuid}@${ipOrDomain}:${port}?encryption=${encryption}&security=tls&sni=${cfhostt}&fp=firefox&alpn=h2&insecure=0&allowInsecure=0&type=xhttp&host=${cfhostt}&path=${path}&mode=auto&extra={"xPaddingObfsMode":true,"xPaddingKey":"x_padding","xPaddingHeader":"Referer","xPaddingPlacement":"queryInHeader","xPaddingMethod":"tokenish","xmux":{"maxConcurrency":"16-32","cMaxReuseTimes":0,"hMaxReusableSecs":"1800-3000","hKeepAlivePeriod":0}}#${addressid}`;
